...
export const drawUrl = (id, remaining) =>
  `https://deckofcardsapi.com/api/deck/${id}/draw/?count=${remaining}`

export const fetchDeck = async () => {
  const { deck_id, remaining } = await fetchApi(deckUrl)
  const cards = fetchApi(drawUrl(deck_id, remaining)).then((data) => data.cards)
  return cards
}
